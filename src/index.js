...
import { startSportsReminder } from './sports-reminder.js';
...
client.once(Events.ClientReady, (readyClient) => {
  ...
  startSportsReminder(readyClient);
    ...
    if (interaction.commandName === 'keys') {
      ...
      withDb((db) => {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          apiPool.push(db.apiKeys[keyId]);
          cumulativeCalls += (db.apiKeys[keyId].successCount || 0) + (db.apiKeys[keyId].errorCount || 0);
        }
        activeIndex = db.n9Router.activeIndex || 0;
        return db;
      });
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          [
            ...
            `📊 **Total Key Pool:** \`5\` Stands | **Cumulative Calls:** \`${cumulativeCalls}\` calls`,
            `\u200b`
          ].join('\n')
        )
      ...
      for (let i = 0; i < 5; i++) {
        const stats = apiPool[i];
        const standName = standNames[i] || `Stand ${i + 1}`;
        
        let statusText = '🟢 **Active (Ready)**';
        if (stats.isRateLimited && stats.rateLimitReset) {
          const remainingSecs = Math.max(0, Math.round((new Date(stats.rateLimitReset).getTime() - Date.now()) / 1000));
          if (remainingSecs === 0) {
